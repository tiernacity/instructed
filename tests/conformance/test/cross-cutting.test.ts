...
async function appendOne(streamUuid: string, e: AppendOptions): Promise<{ event_id: string }> {
  ...
  await pool.query(`SELECT * FROM instructed.append_to_stream($1, 'any', NULL, $2::jsonb)`, [
    streamUuid,
...
async function readFirst(streamUuid: string): Promise<ReadRow> {
  const r = await pool.query<{
    ...
    event_number: string
  }>(`SELECT * FROM instructed.read_stream($1, 0, 100)`, [streamUuid])
...
describe('streams — first-class identity', () => {
  ...
  test('two distinct stream_uuids carry independent events and versions', async () => {
    ...
    assert.deepEqual(
      all.map((r) => r.stream_uuid),
      [a, a, b],
    )
    assert.deepEqual(
      all.map((r) => r.stream_version),
      [1n, 2n, 1n],
    )
  })
  ...
  test('no procedure exposes internal stream_id in its return columns', async () => {
    ...
    assert.equal(
      offenders.length,
      0,
      `procedures exposing internal stream_id: ${offenders.map((o) => o.proname).join(', ')}`,
    )
  })
