...
async function claimSubscription(
  ...
  await q.query(`SELECT * FROM instructed.claim_subscription($1, $2, $3, $4)`, [
    ALL,
    SUB,
    worker,
    leaseSeconds,
...
async function appendN(
  ...
  return r.rows.map((row) => BigInt(row.en)).sort((a, b) => Number(a - b))
}
...
async function claim(
  ...
  const r = await q.query<ClaimRow>(`SELECT * FROM instructed.claim_work_item($1, $2, $3, $4)`, [
    ALL,
    SUB,
    worker,
    leaseSeconds,
...
describe('SUB-A slice 2 — route_batch', () => {
  ...
  test('atomicity: a failing batch leaves cursor and inserts unchanged', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT * FROM instructed.route_batch($1, $2, $3, $4, $5::jsonb)`, [
          ALL,
          SUB,
          WORKER,
          e1.toString(),
    ...
    const items = await pool.query(`SELECT 1 FROM instructed.subscription_work_items`)
  ...
  test('malformed inputs raise 22023', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT * FROM instructed.route_batch($1, $2, $3, 0, 'null'::jsonb)`, [
          ALL,
          SUB,
          WORKER,
        ]),
...
describe('SUB-A slice 2 — claim_work_item', () => {
  ...
  async function seedRouted(decisions: Array<{ pk: string }>): Promise<bigint[]> {
...
describe('SUB-A slice 5 — extend_work_item_claim', () => {
  ...
  async function seedAndClaim(worker = WORKER): Promise<{ en: bigint; expiresBefore: Date }> {
...
describe('SUB-A slice 2 — complete_work_item_projection', () => {
  ...
  test('missing row raises IS030 (takeover already completed)', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT instructed.complete_work_item_projection($1, $2, $3, $4, $5)`, [
          ALL,
          SUB,
          WORKER,
          'p1',
...
describe('SUB-A slice 2 — complete_pm_instance', () => {
  ...
  test('DELETEs snapshot AND all work items for the partition in one tx', async () => {
    ...
    }>(`SELECT * FROM instructed.complete_pm_instance($1, $2, $3, $4)`, [ALL, SUB, 'pm-A', 'PM-A'])
    ...
    const snap = await pool.query(`SELECT 1 FROM instructed.snapshots WHERE source_uuid = 'PM-A'`)
    assert.equal(snap.rowCount, 0)
    // pm-B is untouched.
  ...
  test('atomicity: a failing call rolls back both deletes', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT * FROM instructed.complete_pm_instance($1, 'no-such', $2, $3)`, [
          ALL,
          'pm-A',
          'PM-A',
        ]),
...
describe('SUB-A slice 2 — fail_work_item', () => {
  ...
  test('non-claimant raises IS030', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT instructed.fail_work_item($1, $2, $3, $4, $5, NULL)`, [
          ALL,
          SUB,
          'intruder',
          'p1',
...
describe('SUB-A slice 2 — is_subscription_caught_up', () => {
  ...
  test('routed-then-failed => false (failed blocks catch-up)', async () => {
    ...
    await claim(pool)
    await pool.query(`SELECT instructed.fail_work_item($1, $2, $3, $4, $5, NULL)`, [
      ALL,
      SUB,
      WORKER,
      'p',
...
describe('SUB-A slice 7 — list_pm_rebuild_events', () => {
  ...
  async function routeAndDone(
    ...
    await pool.query(`SELECT instructed.route_batch($1, $2, $3, $4, $5::jsonb)`, [
      ...
      JSON.stringify([{ partition_key: partitionKey, event_number: Number(eventNumber) }]),
    ])
    await claim(pool)
    await pool.query(
      `SELECT instructed.complete_work_item_pm(
  ...
  test("returns only 'done' rows for the partition, ordered by event_number", async () => {
    ...
    await pool.query(`SELECT instructed.route_batch($1, $2, $3, $4, $5::jsonb)`, [
      ...
      WORKER,
      ens[4].toString(),
      JSON.stringify([{ partition_key: 'pm-A', event_number: Number(ens[4]) }]),
    ])
  ...
  test('rejects invalid parameters with 22023', async () => {
    ...
    await rejectsWithCode(
      () =>
        pool.query(`SELECT * FROM instructed.list_pm_rebuild_events($1, $2, NULL, 0)`, [ALL, SUB]),
      '22023',
    )
  })
})
