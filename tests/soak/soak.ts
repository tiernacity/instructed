...
import { forwarderPmDefinition, forwarderRouteFn, newForwarderCounters } from './domain.ts'
...
async function waitForDrain(
  ...
  while (Date.now() < deadline) {
    ...
    for (const sub of subscriptions) {
      ...
      try {
        const caughtUp = await client.isSubscriptionCaughtUp(sub.stream, sub.name, target)
        if (!caughtUp) {
          allCaught = false
          break
        }
...
async function main(): Promise<number> {
  ...
  try {
    ...
    const triggerStreams = Array.from({ length: cli.triggers }, () => randomUUID())
    ...
    stop.aborted = true
    await Promise.all([...workloadTasks, samplerTask, respawnTask, stealTask])
    ...
    const drainState = await snapshotDrainState(pool, subscriptions, drainCompleted)
    ...
    const report = await runFinalChecks(checkCtx, samplerState, balanceProj.balances, {
    ...
    if (report.inconclusive.length > 0) {
      console.log('')
      console.log(
        `INCONCLUSIVE (${report.inconclusive.length}) — drain did not complete; these only hold at quiescence:`,
      )
