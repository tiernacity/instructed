...
function ev(type: string, data: Record<string, unknown> = {}, extra: Record<string, unknown> = {}) {
...
describe('Client.appendToStream', () => {
  ...
  test('expected.exact mismatch raises WrongExpectedVersion (IS001)', async () => {
    ...
    await assert.rejects(
      ...
      (err) => {
        ...
        return err.code === 'IS001' && err.actualVersion === 1n && err.expectedVersion === 99n
      },
    )
  })
  ...
  test('duplicate event_id across appends raises DuplicateEvent (IS004)', async () => {
    ...
    const s2 = randomUUID()
    await assert.rejects(
      () => client.appendToStream(s2, expected.noStream, [{ event_id: id, type: 'B', data: {} }]),
