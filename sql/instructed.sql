...
--   claim_subscription, release_subscription,
--   delete_subscription,
...
-- the PM's SDK-internal snapshot+advance transaction alongside the
-- work-item completion step (D-0016 / PM-023; D-0016 limits the
...
-- its lease; that worker's next call (route_batch or claim_work_item)
-- will fail with IS020, which is the contract's signal to stop.
--
-- Inputs:
--   p_stream_uuid       text
--   p_subscription_name text
...
--
-- A lease heartbeat. If this call fails with
