...
import {
  readTransfers,
  type TransferRow,
} from "../src/projections/transfers/queries.ts";

const POLL_INTERVAL_MS = 1_000;
const RECENT_TRANSFERS = 10;

function renderBalances(
  rows: { stream: string; owner: string | null; balance: number }[],
): string {
  if (rows.length === 0) return "  (no accounts yet)";
  const w = rows.reduce(
    (m, r) => Math.max(m, (r.owner ?? r.stream).length),
    0,
  );
  return rows
    .map((r) => {
...
function renderTransfers(rows: TransferRow[]): string {
  ...
  return rows
    .map((r) => {
      ...
      const status =
        r.status === "failed" ? `failed (${r.reason ?? "?"})` : r.status;
    ...
    const timer = setInterval(() => {
      ...
      tick(pool).catch((err) => {
        process.stderr.write(
          `monitor tick failed: ${(err as Error).message}\n`,
        );
      });
    }, POLL_INTERVAL_MS);
